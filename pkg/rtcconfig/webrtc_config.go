...
func NewWebRTCConfig(rtcConf *RTCConfig, development bool) (*WebRTCConfig, error) {
	...
	useExternalIP := rtcConf.UseExternalIP || rtcConf.UseNodeIPAsExternalIP

	// force it to the node IPs that the user has set
	if rtcConf.NodeIP != "" && (useExternalIP || len(rtcConf.NAT1to1IPs) != 0 || !rtcConf.NodeIPAutoGenerated) {
		if useExternalIP {
	...
	if rtcConf.UseICELite {
		s.SetLite(true)
	} else if (rtcConf.NodeIP == "" || rtcConf.NodeIPAutoGenerated) && !rtcConf.UseExternalIP && !rtcConf.UseNodeIPAsExternalIP && len(rtcConf.NAT1to1IPs) == 0 {
	...
	for _, ip := range localIPs {
		...
		go func(localIP string) {
			...
			if rtcConf.UseNodeIPAsExternalIP {
				addrCh <- ipmapping{externalIP: rtcConf.NodeIP, localIP: localIP}
				return
			}
