...
func NewWebRTCConfig(rtcConf *RTCConfig, development bool) (*WebRTCConfig, error) {
	...
	if rtcConf.NodeIP != "" && (rtcConf.UseExternalIP || len(rtcConf.ExternalProvidedIP) != 0 || !rtcConf.NodeIPAutoGenerated) {
		if rtcConf.UseExternalIP {
			...
			nat1to1IPs = ips
		} else if len(rtcConf.ExternalProvidedIP) != 0 {
			logger.Infow("using externalProvidedIP")
			ips, newFilter, err := getNAT1to1IPsForConf2(rtcConf, ipFilter)
			if err != nil {
				return nil, err
			}
			ipFilter = newFilter
	...
	if rtcConf.UseICELite {
		s.SetLite(true)
	} else if (rtcConf.NodeIP == "" || rtcConf.NodeIPAutoGenerated) && !rtcConf.UseExternalIP && len(rtcConf.ExternalProvidedIP) == 0 {
...
func getNAT1to1IPsForConf2(rtcConf *RTCConfig, ipFilter func(net.IP) bool) ([]string, func(net.IP) bool, error) {
	localIPs, err := GetLocalIPAddresses(rtcConf.EnableLoopbackCandidate, nil)
	if err != nil {
		return nil, ipFilter, err
	}
	...
	for _, ip := range localIPs {
		...
		go func(localIP string) {
			defer wg.Done()
			addr := rtcConf.ExternalProvidedIP
			addrCh <- ipmapping{externalIP: addr, localIP: localIP}
			return
		}(ip)
	}
